...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        expression = expr.map(lambda x: scheme_eval(x, env))
        validate_procedure(expression.first)
        return scheme_apply(expression.first, expression.rest, env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    while expressions != nil:
        last_value = scheme_eval(expressions.first, env)  # replace this with lines of your own code
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        formals = target.rest
        variable = target.first
        validate_formals(formals)
        body = expressions.rest
        procedure = LambdaProcedure(formals, body, env)
        env.define(variable, procedure)
        return variable
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    if expressions.rest != nil:
        return expressions.first + do_quote_form(expressions.rest, env)
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        return scheme_eval(expressions.first, env, result)
